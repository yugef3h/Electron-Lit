...
const { LIT_OPTION, CONFIG_GET } = require('../const');
global[LIT_OPTION].configPath = DEFAULT_PATH;
const config = {};
let hasInit = false;
let appName = '';
// 获取环境变量参数
...
const getConfig = global[CONFIG_GET] = function () {
    if (hasInit === false) {
        ...
        const baseLocalConfig = getLocalBaseConfig(global[LIT_OPTION].configPath);
        const localConfig = getLocalConfig(global[LIT_OPTION].configPath, env);
...
const setOption = function(params = {}) {
    ...
    global[LIT_OPTION].configPath = params.configPath;
    return getConfig();
}
